...
- Stage 3 (manual UI test): PASS on `git remote -v`. The permission
  card shows the project's working-directory path as a cwd header,
  `git remote -v` as the command label, and the three permission
  buttons. Confirmed visually that this matches the spec.
