...
## Phase 3: opencode + Zed integration

(Wired on branch `opencode-gpt-oss-120b`.)

Once 128K performance was settled, the next question was whether the
model could actually be *used* through Zed/opencode — not just curl'd
directly. Three issues had to be resolved:

### 1. The launcher would OOM-kill the experiment

`opencode-session.sh` and `2gpu-launch.sh` both unconditionally start
the four standard llama-* units on every Zed launch. If the experiment
was already loaded on the 7900 XTX, llama-primary's start would:
- Try to load GLM into a card with ~3 GB free
- Fail the `llama_params_fit` check
- Cascade host-memory pressure as the loader unwound
- Trigger the OOM killer, which targeted the largest RSS process —
  the experiment (54 GB peak RSS)

Observed live during the first Zed test of the integration. Fix:
both launchers now check `systemctl is-active --quiet
llama-primary-experiment` at startup and substitute it for
llama-primary if active. Mutually exclusive on Vulkan0 / GPU memory,
treated as swap targets at the workflow level.

### 2. opencode model-id pattern matching

After the OOM was fixed, the model loaded fine and Zed could pick it
in the model picker — but it generated 60+ lines of working Python in
chat instead of writing files. Direct test against the llama-server
endpoint with explicit tool definitions confirmed the model itself
emits tool_calls correctly. So the issue was upstream: opencode wasn't
sending tool definitions in its request.

Root cause: opencode pattern-matches the model id internally to enable
GPT-OSS-specific request handling. Strings inside the binary include
`gpt-oss-120b`, `gpt-oss-20b`, `harmony` etc. as recognized ids. Our
model id was the GGUF filename
(`gpt-oss-120b-mxfp4-00001-of-00003.gguf`) — close, but not a match —
so opencode fell back to a generic openai-compatible flow with no
tools attached.

Fix: pass `--alias gpt-oss-120b` to llama-server so `/v1/models`
reports the canonical id, and use `gpt-oss-120b` as the model key in
the opencode template.

After the alias fix, GPT-OSS-120B in Zed correctly:
- Emits tool_calls instead of code blocks
- Triggers the opencode permission card (matches `permission.edit: "ask"`)
- Writes the file when approved

### 3. Tooling caveats discovered during integration

- **Harmony channels:** GPT-OSS emits private reasoning to
  `reasoning_content` and final answer to `content`. With small
  `max_tokens` budgets, the reasoning can consume the full budget
  and `content` returns empty. The opencode template declares
  `output: 16384` to avoid this; Zed's defaults are generous enough
  that it doesn't bite in normal use.
- **Provider duplication in Zed picker:** Visual confusion only —
  opencode reports 10 unique models and Zed groups them oddly with
  truncation. Pick the full-width entry.

Final result: the model is fully integrated. Zed's model picker
shows it as `chat (experimental)/GPT-OSS-120B 128K`; pick it, ask
the model to write a file, approve the permission card, file gets
written. Library MCP tools follow the same path (architecturally
provider-agnostic, both fire through the chat-completion tool-call
channel).
