...
- Anthropic also reports multi-agent research used **~15x more tokens** than a single-turn chat for comparable tasks; they scope the pattern to tasks "where the value of the answer is high enough to pay for it." <https://claude.com/blog/building-multi-agent-systems-when-and-how-to-use-them>
...
- Cost scales linearly with turns x models (every turn pays full prefill for every replayer).
...
- GroupDebate and S^2-MAD (efficiency variants). <https://arxiv.org/html/2409.14051>, <https://arxiv.org/html/2502.04790v2>

**Measured outcomes.**
- Du et al.: on GSM8K, 3 agents x 2 rounds reach **85.0%** vs. single-agent **77.0%** (+8pp); MMLU, biographies, and chess show 5-10% absolute gains. <https://arxiv.org/abs/2305.14325>
...
- S^2-MAD: up to **94.5% token-cost reduction** vs. baseline MAD with **<2% accuracy loss**. <https://arxiv.org/html/2502.04790v2>
...
**Reported failure modes.**
- Quadratic (or worse) token cost in rounds x agents. Explicitly documented in every debate paper reviewed. <https://arxiv.org/html/2409.14051>
...
|---|---|---|---|---|---|
| 1. Orchestrator-Owns-Thread | Lead agent's context + digested subagent outputs | Anthropic: +90.2% on internal research eval; 15x token cost | Sub-result compression loses nuance; hierarchy latency | Production (Anthropic, Claude Code, LangGraph Supervisor) | Parallel research, multi-tool, high-value answers |
...
| 7. Consensus / Debate Aggregation | Aggregated answer of N parallel models | Du et al. +8pp GSM8K; A-HMAD +4-6pp; S^2-MAD -94.5% tokens | Majority-is-wrong; roundxagent cost; single-turn focus | Research-mature; limited production | Single-turn factuality / arithmetic QA |
...
5. **Failure taxonomy is new.** The MAST taxonomy (Cemri et al. 2025) is the first empirically grounded catalogue of multi-agent failure modes (14 modes across 3 categories, kappa=0.88 inter-annotator). It came out in 2025 and is still absorbing. <https://arxiv.org/abs/2503.13657>
...
- S^2-MAD (arXiv 2502.04790). <https://arxiv.org/html/2502.04790v2>
