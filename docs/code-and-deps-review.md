...
- **L112-117 -- phase regexes are brittle.** They match exact llama.cpp log strings (`"ggml_cuda_init: found"`, `"llama_context: constructing llama_context"`, `"llama_kv_cache: size ="`, `"srv    load_model: initializing slots"`, `"server is listening"`). Upstream log strings have rotated ~3x across 2025. When you bump llama.cpp, *first* run this script once, then fix any columns that show `?`. Worth documenting at the top.
